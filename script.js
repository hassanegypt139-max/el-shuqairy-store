...
(function() {
    ...
    const i18n = {
        ar: {
            ...
            heroDesc: 'اكتشف أحدث العروض والخصومات من أمازون مصر. منتجات مختارة بعناية في مختلف الفئات بأسعار تنافسية.',
            statProducts: 'منتج متاح',
            statDiscount: 'خصم حتى',
            statStores: 'متجر موثوق',
            filterStore: 'المتجر',
            ...
            footerAffiliate: 'رابط أفيليت أمازون: hassanshukair-21',
            ...
            trustCard2Desc: 'نراقب أسعار أمازون يومياً ونعرض لك أرخص العروض المتاحة. نوفر لك كوبونات خصم حصرية تساعدك على التوفير أكثر في كل عملية شراء.',
            trustCard3Title: 'تسوق آمن 100%',
            trustCard3Desc: 'الشراء يتم مباشرة من أمازون مصر، أكبر متجر إلكتروني موثوق عالمياً. نحن نوفر لك أفضل العروض وأنت تشتري بأمان تام من المتجر الرسمي.',
            trustCard4Title: 'تقييمات حقيقية من مستخدمين',
            trustCard4Desc: 'كل منتج مرفق بتقييمات ومراجعات حقيقية من مشترين فعليين على أمازون. تقييمات شفافة تساعدك على اتخاذ قرار الشراء الصحيح.',
        ...
        en: {
            ...
            logoSub: 'Amazon Egypt',
            heroTitle: 'Shop the <span class="gold">Best Products</span> at Lowest Prices',
            heroDesc: 'Discover the latest deals and discounts from Amazon Egypt. Curated products across categories at competitive prices.',
            statProducts: 'Products Available',
            statDiscount: 'Discount Up To',
            statStores: 'Trusted Store',
            filterStore: 'Store',
            ...
            footerAffiliate: 'Amazon Affiliate: hassanshukair-21',
            ...
            trustCard2Desc: 'We monitor Amazon prices daily and show you the best available offers. We provide exclusive discount coupons to help you save more on every purchase.',
            trustCard3Title: '100% Secure Shopping',
            trustCard3Desc: 'Purchases are made directly from Amazon Egypt, the largest globally trusted online store. We provide the best offers and you buy safely from the official store.',
            trustCard4Title: 'Real User Reviews',
            trustCard4Desc: 'Every product includes real ratings and reviews from actual buyers on Amazon. Transparent ratings to help you make the right purchase decision.',
    ...
    function initCampaign(remoteData) {
        ...
        if (remoteData && remoteData.campaign && remoteData.campaign.active) {
            ...
            campaignState.link = 'https://www.amazon.eg';
        }
    ...
    function renderProducts() {
        ...
        products.forEach((p, index) => {
            const storeClass = p.store === 'amazon' ? 'amazon' : 'amazon';
            const storeName = p.store === 'amazon' ? 'Amazon' : 'Amazon';
    ...
    function openModal(productId) {
        ...
        const storeClass = product.store === 'amazon' ? 'amazon' : 'amazon';
        const storeName = product.store === 'amazon' ? 'Amazon' : 'Amazon';
        ...
        DOM.modalBuyBtn.className = 'btn-buy amazon-btn';
    ...
    function renderCart() {
        ...
        state.cart.forEach(item => {
            ...
            total += itemTotal;
            const storeClass = item.store === 'amazon' ? 'amazon' : 'amazon';
            const storeName = item.store === 'amazon' ? 'Amazon' : 'Amazon';
    ...
    function mergeProducts(remoteData) {
        let amazon = [];
        // Noon products kept in JSON structure for future compatibility

        if (remoteData && remoteData.amazon) {
            amazon = remoteData.amazon.map(p => ({ ...p, store: 'amazon' }));
        }

        // Only Amazon products are loaded into the store
        const all = [...amazon];
    ...
    async function fetchProducts() {
        ...
        if (state.allProducts.length > 0) {
            ...
            // Only Amazon cached; noon kept empty for JSON compatibility
